...
class InformationBottleneck_trend(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.in_d = input_dim  # 输入维度
        self.out_d = output_dim  # 输出维度
        ...
        # 构建均值和标准差层
        self.fc_mu = nn.Linear(output_dim, output_dim)  # 均值层
        self.fc_std = nn.Linear(output_dim, output_dim)  # 标准差层
...
def apply_fft(trend_output):
    ...
    """
    fft_result = fft.fft(trend_output, dim=1)
    return fft_result

def inverse_fft(trend_output_fft):
    """
    对趋势性数据经过FFT变换后进行逆变换。
    ...
    """
    inverse_result = fft.ifft(trend_output_fft, dim=1)
...
class Model(nn.Module):
    ...
    def __init__(self, configs, individual=False):
        ...
        # MLP 配置
        self.mlp_input_dim = int(self.seq_len/2)  # MLP的输入维度，根据实际情况调整
        self.mlp_hidden_dim = self.seq_len  # MLP的隐藏层维度
        self.mlp_output_dim = int(self.seq_len/2)  # MLP的输出维度，根据实际情况调整

        self.imlp_input_dim = self.dec_in  # MLP的输入维度，根据实际情况调整
        self.imlp_hidden_dim = self.dec_in * 2  # MLP的隐藏层维度
        self.imlp_output_dim = self.dec_in  # MLP的输出维度，根据实际情况调整

        self.out_mlp_input_dim = self.seq_len  # MLP的输入维度，根据实际情况调整
        self.out_mlp_hidden_dim = self.seq_len * 5  # MLP的隐藏层维度
        self.out_mlp_output_dim = self.pred_len  # MLP的输出维度，根据实际情况调整
        ...
        self.mlp = nn.Sequential(
            ...
            nn.Linear(self.mlp_hidden_dim*2, self.mlp_output_dim)
        )

        self.imlp_2 = nn.Sequential(
            nn.Linear(self.imlp_input_dim, self.imlp_hidden_dim),
            # nn.ReLU(),
            nn.Linear(self.imlp_hidden_dim, self.imlp_hidden_dim * 2),
            nn.Linear(self.imlp_hidden_dim * 2, self.imlp_output_dim)
        )

        self.imlp_1 = nn.Sequential(
            nn.Linear(self.imlp_input_dim, self.imlp_hidden_dim),
            # nn.ReLU(),
            nn.Linear(self.imlp_hidden_dim, self.imlp_hidden_dim * 2),
            nn.Linear(self.imlp_hidden_dim * 2, self.imlp_output_dim)
        ...
        self.out_mlp = nn.Sequential(
            ...
            #nn.Linear(self.out_mlp_input_dim, self.out_mlp_output_dim),
            #nn.GELU(),
        ...
        if self.individual:
            ...
            self.Linear_Seasonal = nn.Linear(self.pred_len, self.pred_len)
            self.Linear_Trend = nn.Linear(self.pred_len, self.pred_len)
    ...
    def encoder(self, x):
        ...
        if self.individual:
            ...
            #trend_output_fft = self.IFFT(fft_processed.permute(0, 2, 1))+trend_init.permute(0, 2, 1)
            #trend_output_fft = self.IFFT(fft_processed)+trend_init.permute(0, 2, 1) torch.zeros_like(seasonal_output+seasonal_init)
        #final_output = self.IDWT(seasonal_output, trend_output_fft.permute(0, 2, 1))+x.permute(2, 0, 1)
        final_output_ = self.IDWT(trend_init + trend_output, seasonal_init+seasonal_output) #+ x.permute(2, 0, 1)
        # x = seasonal_output + trend_output  trend_output_fft.permute(0, 2, 1)+   +seasonal_output
        #final_output = self.pre_1(final_output) #wu!!!!
        #final_output = self.pre_2(final_output)
        #final_output = self.pre_3(final_output)
